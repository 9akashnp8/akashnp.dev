...
<article class="prose prose-slate dark:prose-invert">
    ...
    Applications for this response type can be Chat Apps (eg: [chat.openai.com](https://chat.openai.com)), Stock Price Trackers etc.
    ...
    <sub><sup>The diagram above was created with [Erasers](https://eraser.io) ["Diagram As Code"](https://docs.eraser.io/docs/diagram-as-code) feature.</sup></sub>

    We'll be using the `EventSourceResponse` from sse_starlette, this response type implements the Server Sent Event protocol and all it needs is an async generator, When the client initializes an EventSource connection to this endpoint, the response classes invokes the aysnc generator and streams the data recieved returned by the async generator.
